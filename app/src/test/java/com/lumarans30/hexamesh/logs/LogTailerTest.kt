...
class LogTailerTest {
    ...
    private fun logFile(contents: String = ""): File =
        folder.newFile("llama-server.log").apply { writeText(contents) }

    private fun List<TailEvent>.texts(): List<String> =
        filterIsInstance<TailEvent.Line>().map { it.text }

    @Test
    fun `emits lines already in the file`() = runTest {
    ...
    @Test
    fun `follows lines appended after it starts`() = runTest {
        val file = logFile("first\n")
        val events = mutableListOf<TailEvent>()
    ...
    @Test
    fun `waits for the file to appear`() = runTest {
    ...
    @Test
    fun `signals reset and re-reads when the file is truncated`() = runTest {
    ...
    @Test
    fun `strips carriage returns from CRLF lines`() = runTest {
        val file = logFile("a\r\nb\r\n")
        val events = mutableListOf<TailEvent>()
    ...
    @Test
    fun `holds a trailing partial line until it is terminated`() = runTest {
